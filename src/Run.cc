...
{
    //casto a stringa un po' piu' leggibile
    std::string s_ene = std::to_string(fEkin);
    ...
    std::string fileName = "hardXXX"+s_ene+"MeV.txt";
    ...
    for (itn = fParticleDataMap.begin(); itn != fParticleDataMap.end(); itn++) { 
        ...
        G4double eMax = data.fEmax;    
        if (print)         
            outputF << "  " << std::setw(13) << name << ": " << std::setw(7) << count
